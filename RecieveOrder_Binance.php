...
$checkdata = checkwebsite('Binance ', $apiKey, $secretKey);
...
logger("Webhook Received (Binance) | $tvOrder");
    ...
    try{
      if ($orderType === 'limit') {
        if($limitPrice && empty($limitPercentageOffset)){
          logger("Placing Order (Binance) | Pair: $symbol, Side:  $side, Type: $orderType, Qty: $quantity, Price: $limitPrice");
          while (!$success && $attempt <= $retryAttempts) {
              try {
                  ...
                  logger("Limit Order Placed (Binance) | OrderID: $orderID, Status: $status");
                  $success = true;
              } catch (Exception $e) {
                  logger("Error Placing Order (Binance) |". $e->getMessage());
                  sleep($retryDelay);
                  $attempt++;
              }
          }
          ...
          logger("Placing Order (Binance) | Pair: $symbol, Side:  $side, Type: $orderType, Qty: $quantity, PercentageOffset: $limitPercentageOffset, Price: $limitPrice");
          while (!$success && $attempt <= $retryAttempts) {
              try {
                  $order = $exchange->create_limit_order($coin['id'], $side, $quantity, $limitPrice);
                  $orderID = $order['id'];
        ...
        while (!$success && $attempt <= $retryAttempts) {
            try {
                ...
                $orderID = $order['id'];
                $status = json_encode($order);
                logger("Market Order Placed (Binance) | OrderID: $orderID, Status: $status");
                $success = true;
            } catch (Exception $e) {
                logger("Error Placing Order (Binance) |". $e->getMessage());
                sleep($retryDelay);
