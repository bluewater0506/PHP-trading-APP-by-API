...
$checkdata = checkwebsite('Kucoin ', $apiKey, $secretKey);
...
logger("Webhook Received (KuCoin) | $tvOrder");
...
if($tvOrder){
  ...
  if($symbol !== null && $direction !== null && $orderSide !== null && $orderType !== null && $quantity !== null && $coin !== null){
    $baseCoin = $coin['settleId'];
    $pos = 0;
    $getPositions = $exchange->fetch_positions();
    if(!empty($getPositions)){
      foreach ($getPositions as $position) {
        if ($position['info']['symbol'] === $coin['id']) {
            $pos = 1;
            $positionInfo = $position['info'];
            $id = $positionInfo['id'];
            $qty = $positionInfo['currentQty'];
            if($qty>0 && $side === 'sell' || $qty<0 && $side === 'buy'){
              $baseBalance = abs($positionInfo['currentQty']);
              $quantity = trim($quantity, '%');
              $quantity = $baseBalance * ($quantity / 100);
              $quantity = round($quantity);
             }else{
               $balanceReq = $exchange->fetch_balance();
               $baseBalance = $balanceReq["$baseCoin"]['free'];
               $quantity = trim($quantity, '%');
               $quantity = $baseBalance * ($quantity / 100);
               $ticker = $exchange->fetch_ticker($coin['id']);
               $lastPrice = $ticker['last'];
               $quantity = round(($quantity / $lastPrice)/$coin['contractSize']);
             }
        }
      }
    }
    ...
    try{
      if ($orderType === 'limit') {
        if($limitPrice && empty($limitPercentageOffset)){
          logger("Placing Order (KuCoin) | Pair: $symbol, Side:  $side, Type: $orderType, Qty: $quantity, Price: $limitPrice");
          while (!$success && $attempt <= $retryAttempts) {
              try {
                  $order = $exchange->create_order($coin['id'], $orderType,$side, $quantity, $limitPrice, $params);
                  $orderID = $order['id'];
                  $status = json_encode($order);
                  if(!empty($orderID)){
                    logger("Limit Order Placed (KuCoin) | OrderID: $orderID, Status: Order Placed");
                    $success = true;
                  } else{
                    logger("Error Placing Order (KuCoin) |");
                    sleep($retryDelay);
                    $attempt++;
                  }
              } catch (Exception $e) {
                  logger("Error Placing Order (KuCoin) |". $e->getMessage());
                  sleep($retryDelay);
                  $attempt++;
              }
          }
          ...
          logger("Placing Order (KuCoin) | Pair: $symbol, Side:  $side, Type: $orderType, Qty: $quantity, PercentageOffset: $limitPercentageOffset, Price: $limitPrice");
          while (!$success && $attempt <= $retryAttempts) {
              try {
                  $order = $exchange->create_order($coin['id'], $orderType,$side, $quantity, $limitPrice, $params);
                  $orderID = $order['id'];
        ...
        while (!$success && $attempt <= $retryAttempts) {
            try {
                $order = $exchange->create_order($coin['id'],'market', $side, $quantity,null, $params);
                $orderID = $order['id'];
                $status = json_encode($order);
                if(!empty($orderID)){
                  logger("Market Order Placed (KuCoin) | OrderID: $orderID, Status: Order Placed");
                  $success = true;
                } else{
                  logger("Error Placing Order (KuCoin) |");
                  sleep($retryDelay);
